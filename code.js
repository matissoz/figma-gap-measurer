...
const LABEL_COLOR = { r: 1, g: 0.22, b: 0.37 }; // #FF3860
const LINE_COLOR = { r: 1, g: 0.22, b: 0.37 };
const INFO_COLOR = { r: 0.15, g: 0.15, b: 0.15 }; // #262626
const LEADER_COLOR = { r: 0.4, g: 0.4, b: 0.4 };
...
function filterToNearestNeighbors(allBounds) {
    const gaps = [];
...
// ── Drawing primitives ──
function createLine(x1, y1, x2, y2, color = LINE_COLOR, dashed = true) {
...
function createEndCap(x, y, direction) {
...
function createGapLabel(text, x, y, direction) {
...
function createInfoTag(lines, bgColor, x, y) {
    const frame = figma.createFrame();
...
function createLeaderLine(fromX, fromY, toX, toY, group) {
...
// ── Element outline ──
function drawElementOutline(bounds, group) {
...
// ── Helpers ──
function getOrCreateMeasurementGroup() {
    clearMeasurements();
...
function getColorsFromNode(node) {
    const colors = [];
    // Resolve fill style name
    const fillStyleName = "fillStyleId" in node ? getStyleName(node.fillStyleId) : null;
    // Resolve stroke style name
    const strokeStyleName = "strokeStyleId" in node ? getStyleName(node.strokeStyleId) : null;
    if ("fills" in node) {
...
function getTypographyFromNode(node) {
    const info = [];
    if (node.type !== "TEXT")
        return info;
    const textNode = node;
    // Check for text style name
    const textStyleName = "textStyleId" in textNode ? getStyleName(textNode.textStyleId) : null;
    ...
    if (lineHeight !== figma.mixed) {
        if (lineHeight.unit === "PIXELS")
            parts.push(`LH: ${lineHeight.value}px`);
        else if (lineHeight.unit === "PERCENT")
            parts.push(`LH: ${Math.round(lineHeight.value)}%`);
    }
    const fontWeight = textNode.fontWeight;
    if (fontWeight !== figma.mixed)
        parts.push(`W: ${fontWeight}`);
...
function truncate(text, max = MAX_TEXT_PREVIEW) {
    return text.length > max ? text.slice(0, max) + "…" : text;
}
function getVariablesFromNode(node) {
    const vars = [];
    try {
    ...
    if (node.type !== "TEXT" && "findAll" in node) {
        ...
        for (const child of textChildren.slice(0, 6)) {
            ...
            if (childColors.length > 0)
                lines.push(childColors.join(" · "));
            if (childVars.length > 0)
                lines.push(...childVars);
            if (childTypo.length > 0)
                lines.push(...childTypo);
        }
    }
    return lines;
}
...
async function measureSelected() {
    ...
    for (const gap of gaps)
        drawGap(gap, group);
    if (gaps.length === 0) {
        figma.notify("No gaps found");
        group.remove();
    }
    else
        figma.notify(`📏 ${gaps.length} gap${gaps.length > 1 ? "s" : ""}`);
}
async function measureSiblings() {
    const selection = figma.currentPage.selection;
    ...
    for (const sibling of parent.children) {
        if (sibling.id === selected.id || !sibling.visible)
            continue;
    ...
    if (count === 0) {
        ...
        figma.notify(`📏 ${count} gap${count > 1 ? "s" : ""} to siblings & parent`);
}
...
async function annotateAll() {
    ...
    if (selection.length === 1) {
        ...
        if (parent && parent.type !== "PAGE") {
            ...
            for (const gap of gaps) {
                const involvesSelected = (Math.abs(gap.startX - sb.right) < 1 || Math.abs(gap.endX - sb.x) < 1 ||
