...
import { z } from 'zod';
import { parseFigmaUrl, getFilePages, getNode, getFileStyles, exportImage, inspectNode, measureGaps, measureGapsRecursive, formatInspection, formatGaps, formatGapsRecursive, log, } from './figma-api.js';
...
server.tool('inspect_node', 'Inspect a Figma node — returns typography, colors, spacing, layout, and styles for the node and its children. Accepts a Figma URL or file_key + node_id.', {
    ...
    try {
        ...
        if (args.url) {
            ...
            return { content: [{ type: 'text', text: 'Error: Provide either a Figma URL or file_key + node_id' }] };
        }
...
// ── Tool: measure_gaps ──
server.tool('measure_gaps', 'Measure gaps/spacing between children of a Figma frame. Use depth > 0 to recursively measure gaps at ALL levels (sections, cards, etc.) in one call — gives the full spacing picture for an entire page.', {
    ...
    try {
        ...
        if (maxDepth > 0) {
            ...
            return { content: [{ type: 'text', text: `Recursive gaps in "${node.name}" (depth ${maxDepth}):\n\n${text}` }] };
        }
        else {
            ...
            return { content: [{ type: 'text', text: `Gaps in "${node.name}":\n\n${text}` }] };
        }
    }
    catch (err) {
...
server.tool('get_file_structure', 'List all pages and top-level frames in a Figma file. Use this to discover node IDs for further inspection.', {
    ...
    try {
        ...
        if (args.url) {
            ...
            return { content: [{ type: 'text', text: 'Error: Provide either a Figma URL or file_key' }] };
        }
...
// ── Tool: export_image ──
server.tool('export_image', 'Export an image/icon/illustration from Figma and save it to a local path. Exports at 2x scale by default for retina. Supports PNG, JPG, SVG, PDF.', {
...
// ── Tool: optimize_image ──
server.tool('optimize_image', 'Optimize an image for web: converts to WebP (default) or AVIF, resizes if needed, and compresses. Always use this after export_image or when adding any image to the project.', {
