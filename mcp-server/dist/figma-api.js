...
async function figmaFetch(path) {
    ...
    if (!res.ok) {
        ...
        if (res.status === 429) {
            ...
            throw new Error(`Figma API rate limit exceeded (429). ` +
...
// ── File structure ──
export async function getFilePages(fileKey) {
    ...
    if (node.fills) {
        for (const fill of node.fills) {
            ...
            if (fill.type === 'GRADIENT_LINEAR') {
                const stops = fill.gradientStops?.map((s) => `${rgbToHex(s.color.r, s.color.g, s.color.b)} ${Math.round(s.position * 100)}%`).join(' → ');
...
function extractTypography(node) {
    ...
    const parts = [];
    if (style.fontFamily)
        parts.push(`${style.fontFamily} ${style.fontPostScriptName?.split('-').pop() || ''}`);
    if (style.fontSize)
        parts.push(`${style.fontSize}px`);
    if (style.lineHeightPx)
        parts.push(`LH: ${Math.round(style.lineHeightPx)}px`);
    else if (style.lineHeightPercent)
        parts.push(`LH: ${Math.round(style.lineHeightPercent)}%`);
    if (style.fontWeight)
        parts.push(`W: ${style.fontWeight}`);
    if (style.letterSpacing && style.letterSpacing !== 0)
        parts.push(`LS: ${style.letterSpacing}px`);
    if (parts.length > 0)
...
function extractLayout(node) {
    ...
    if (node.layoutMode) {
        ...
        if (node.itemSpacing != null)
            info.push(`Gap: ${node.itemSpacing}px`);
    ...
    if (node.cornerRadius)
        info.push(`Radius: ${node.cornerRadius}px`);
...
function extractStyles(node, fileStyles) {
    const info = [];
    if (node.styles) {
        for (const [type, styleId] of Object.entries(node.styles)) {
            const style = fileStyles[styleId];
            if (style)
                info.push(`${type} style: ${style.name}`);
        }
    }
    return info;
}
export function inspectNode(node, fileStyles = {}, depth = 0, maxDepth = 4, parentBox) {
    ...
    if (node.children && depth < maxDepth) {
        ...
        result.children = node.children
            ...
            .map((child) => inspectNode(child, fileStyles, depth + 1, maxDepth, currentBox));
    }
    return result;
}
export function measureGaps(node) {
    if (!node.children)
        return [];
    ...
    for (let i = 0; i < children.length; i++) {
        for (let j = i + 1; j < children.length; j++) {
            ...
            if (yOverlap > 0) {
                const hGap = Math.abs(Math.min(a.x, b.x) === a.x
                    ? b.x - (a.x + a.width)
                    : a.x - (b.x + b.width));
                if (hGap > 0 && hGap < 500) {
                    gaps.push({
            ...
            if (xOverlap > 0) {
                const vGap = Math.abs(Math.min(a.y, b.y) === a.y
                    ? b.y - (a.y + a.height)
                    : a.y - (b.y + b.height));
                if (vGap > 0 && vGap < 500) {
                    gaps.push({
...
export function measureGapsRecursive(node, depth = 0, maxDepth = 3) {
    const results = [];
...
export function formatGapsRecursive(results) {
    if (results.length === 0)
        return 'No gaps found at any level.';
    return results.map(r => {
        const header = `── "${r.parent}" (${r.parentId}) ──`;
        const gapLines = r.gaps.map(g => `  ${g.direction === 'horizontal' ? '↔' : '↕'} ${g.gap}px: "${g.from}" → "${g.to}"`).join('\n');
        return `${header}\n${gapLines}`;
    }).join('\n\n');
}
// ── Export image ──
export async function exportImage(fileKey, nodeId, format = 'png', scale = 2) {
    const data = await figmaFetch(`/images/${fileKey}?ids=${nodeId}&format=${format}&scale=${scale}`);
...
export function formatInspection(node, indent = 0) {
    const pad = '  '.repeat(indent);
    const lines = [];
    lines.push(`${pad}[${node.type}] ${node.name} (${node.id})`);
    for (const l of node.layout)
        lines.push(`${pad}  ${l}`);
    for (const c of node.colors)
        lines.push(`${pad}  ${c}`);
    for (const t of node.typography)
        lines.push(`${pad}  ${t}`);
...
export function formatGaps(gaps) {
    if (gaps.length === 0)
        return 'No gaps found between children.';
    return gaps.map(g => `${g.direction === 'horizontal' ? '↔' : '↕'} ${g.gap}px: "${g.from}" → "${g.to}"`).join('\n');
}
